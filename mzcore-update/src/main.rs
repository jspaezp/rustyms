...
fn main() {
        ...
        || args.contains(&"psi-mod".to_string())
    {
        ...
        index
            .save_to_static_data_at(std::path::Path::new("mzcore/src/databases/psimod.dat"))
            .unwrap();
        Some(index)
    } else {
        None
    ...
    let resid = if args.is_empty() || args.contains(&"resid".to_string()) {
        ...
        if path.exists() {
            ...
            index
                .save_to_static_data_at(std::path::Path::new("mzcore/src/databases/resid.dat"))
                .unwrap();
            Some(index)
        } else {
            println!(
        ...
        || args.contains(&"xl-mod".to_string())
    {
        ...
        index
            .save_to_static_data_at(std::path::Path::new("mzcore/src/databases/xlmod.dat"))
            .unwrap();
        Some(index)
    } else {
        None
    ...
    if args.is_empty() || args.contains(&"gno".to_string()) || args.contains(&"gnome".to_string()) {
        ...
        index
            .save_to_static_data_at(std::path::Path::new("mzcore/src/databases/gnome.dat"))
            .unwrap();
    }
    if link
    ...
    if let Some(index) = unimod {
        index
            .save_to_static_data_at(std::path::Path::new("mzcore/src/databases/unimod.dat"))
            .unwrap();
    }
}
