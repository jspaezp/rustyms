...
mod tests {
    ...
    fn groups(lines: &[&str]) -> AttributeGroups {
        ...
        for line in lines {
            let (group, attribute, _) = Attribute::parse(line).unwrap();
            groups
                .entry(group)
                .or_default()
                .push((attribute, Context::default().to_owned()));
        }
        groups
    }

    #[test]
    fn later_context_replaces_all_instances() {
        let mut sets = AttributeSets::new();
        sets.insert(
            "all".into(),
            groups(&[
                "MS:1000543|data processing action=first",
                "MS:1000543|data processing action=second",
            ]),
        );
        sets.insert(
            "named".into(),
            groups(&["MS:1000543|data processing action=third"]),
        );

        let resolved = resolve_attribute_sets(
            &groups(&[
                "MS:1003212|library attribute set name=named",
                "MS:1000543|data processing action=fourth",
            ]),
            Some(&sets),
        );
        let values: Vec<_> = resolved[&None]
            .iter()
            .filter(|(attribute, _)| attribute.name.accession == mzcv::curie!(MS:1000543))
            .map(|(attribute, _)| attribute.value.to_string())
            .collect();
        assert_eq!(values, ["fourth"]);
    }

    #[test]
    fn grouped_claim_is_resolved_in_that_group() {
        let mut sets = AttributeSets::new();
        sets.insert(
            "human".into(),
            groups(&["MS:1001469|taxonomy: scientific name=Homo sapiens"]),
        );

        let resolved = resolve_attribute_sets(
            &groups(&["[2]MS:1003212|library attribute set name=human"]),
            Some(&sets),
        );
        assert!(
            resolved[&Some(2)]
                .iter()
                .any(|(attribute, _)| attribute.name.accession == mzcv::curie!(MS:1001469))
        );
